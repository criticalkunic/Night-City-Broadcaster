...
async function pollStatus() {
  try {
    ...
    $('st-format').textContent=st.running?`${st.backend||'unknown'} · ${st.pixel_format||'unknown'} · driver reports ${st.native_fps} FPS${st.exposure!=null?' · exposure '+st.exposure+' ('+(st.exposure_units||'driver units')+'), auto '+st.auto_exposure:''}`:'';
