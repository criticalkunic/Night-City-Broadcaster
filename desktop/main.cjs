...
async function start(){
 ...
 backend=spawn(path.join(root,embedded?'python.exe':process.platform==='win32'?'ncb-server.exe':'ncb-server'),embedded?[path.join(root,'server.py')]:[],{env:{...process.env,NCB_PORT:String(port),NCB_DATA_DIR:app.getPath('userData'),NCB_SESSION_TOKEN:token},stdio:['ignore','pipe','pipe'],windowsHide:true});
 backend.stdout.resume();
 let tail='';backend.stderr.on('data',d=>{tail=(tail+d).slice(-8000);});
 backend.on('error',error=>{dialog.showErrorBox('Could not start Night City Broadcast',error.message);app.quit();});
 backend.on('exit',()=>{if(!quitting){dialog.showErrorBox('Broadcast service stopped',tail||'The service exited unexpectedly.');app.quit();}});
 let ok=false;for(let i=0;i<120;i++){if(await ready()){ok=true;break;}await new Promise(r=>setTimeout(r,500));}
 ...
 window=new BrowserWindow({width:1400,height:950,minWidth:900,minHeight:640,title:'Night City Broadcast',backgroundColor:'#101216',autoHideMenuBar:true,webPreferences:{nodeIntegration:false,contextIsolation:true,sandbox:true}});
 ...
 console.log('Night City Broadcast window loaded');
}
